/*
 * baseline - require loadcell to be step on to turn off the reward signal
...
void setup() {
  ...
  log_event(state,sync_state,duration_hold,weight_lever_threshold);
}

void log_event(String event, bool sync_state, unsigned long duration_hold, float weight_lever_threshold) {
...
bool check_weight(float weight, float weight_threshold) {
  if(weight>weight_threshold)
  {
    ...
    if(event_name != "pressed")
    {
      ...
      {
        event_name = "pressed";
        log_event(event_name,sync_state,duration_hold,weight_lever_threshold);
        }
...
String state_transition() {
  if(state == "iti"){
    ...
    time_difference = millis() - timekeep;
    if(check_weight(weight_lever, weight_loadcell_threshold)){
      digitalWrite(REWARDOUT,LOW);
      timekeep = millis();
      state = "iti";
      log_event(state,sync_state,duration_hold,weight_lever_threshold);
      }
    else if(time_difference >= duration_reward){
      digitalWrite(REWARDOUT,LOW);
      timekeep = millis();
      state = "iti";
      log_event(state,sync_state,duration_hold,weight_lever_threshold);
...
void loop() {
  ...
  log_event("weight_check",sync_state,duration_hold,weight_lever_threshold);
